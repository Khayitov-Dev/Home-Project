...
from restaurants.snippets import generate_unique_slug,slugify
...
class HomePlaceSale(models.Model):
    ...
    def save(self, *args, **kwargs):
        if self.slug:  # edit
            if slugify(self.title) != self.slug:
                self.slug = generate_unique_slug(HomePlaceSale, self.title)
        else:  # create
            self.slug = generate_unique_slug(HomePlaceSale, self.title)
...
class Comment(models.Model):
    user = models.ForeignKey(User, on_delete=models.CASCADE)
    post = models.ForeignKey(HomePlaceSale, on_delete=models.CASCADE, related_name='comments')
